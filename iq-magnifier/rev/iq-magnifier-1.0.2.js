...
(function($) {
	$.fn.iqMagnifier = function(options) {
		...
		var param = $.extend({
			...
			imgPopupPosition	: 'right', // right, left, top, bottom
			imgPopupOffsetX		: 50,
			imgPopupOffsetY		: 0,
		...
		if (!param.imgPopupWidth) param.imgPopupWidth = param.imgMainWidth;
		if (!param.imgPopupHeight) param.imgPopupHeight = param.imgMainHeight;
		if (!param.imgPopupBdWidth) param.imgPopupBdWidth = param.imgMainBdWidth;
		if (!param.imgPopupBdColor) param.imgPopupBdColor = param.imgMainBdColor;
		
		var imgRatio = param.imgMainHeight / param.imgMainWidth;
		var magnifierHeight = param.magnifierWidth * imgRatio;
		...
		$imgPopup.css({
			...
			height: param.imgPopupHeight * param.imgMainHeight / magnifierHeight
		});
		...
		for (i=0 ; i < param.imgOverlaySrc.length ; i++) {
			$imgOverlay.eq(i).attr('src', param.imgOverlaySrc[i]);
			$imgOverlay.eq(i).css({
		...
		function zoom(e) {
			...
			if (e.originalEvent.wheelDelta < 0) {
				if (currMagnifierWidth + param.zoomScale < imgMainWidth) {
					...
					if (y - (currMagnifierHeight + param.zoomScale * imgRatio) * 0.5 < imgMainTop) {
						$magnifier.css({
							top: param.magnifierBdWidth * (-1),
							height: "+=" + param.zoomScale * imgRatio
						});
						$imgMagnifier.css({
							top: 0
						});
					} else if (y + (currMagnifierHeight + param.zoomScale * imgRatio) * 0.5 > imgMainBottom) {
						$magnifier.css({
							top: param.magnifierBdWidth * (-1) + imgMainHeight - currMagnifierHeight - param.zoomScale * imgRatio,
							height: "+=" + param.zoomScale * imgRatio
						});
						$imgMagnifier.css({
							top: (imgMainHeight - currMagnifierHeight - param.zoomScale * imgRatio) * (-1)
						});
					} else {
						$magnifier.css({
							top: param.magnifierBdWidth * (-1) + y - imgMainTop - (currMagnifierHeight + param.zoomScale * imgRatio) * 0.5,
							height: "+=" + param.zoomScale * imgRatio
						});
						$imgMagnifier.css({
							top: (y - imgMainTop - (currMagnifierHeight + param.zoomScale * imgRatio) * 0.5) * (-1)
						});
					}
				} else {
					$magnifier.css({
				...
				if (currMagnifierWidth - param.zoomScale > imgMainWidth * zoomRatio) {
					...
					if (y - currMagnifierHeight * 0.5 + param.zoomScale * imgRatio * 0.5 < imgMainTop) {
						$magnifier.css({
							top: param.magnifierBdWidth * (-1),
							height: "-=" + param.zoomScale * imgRatio
						});
					...
					} else if (y + currMagnifierHeight * 0.5 - param.zoomScale * imgRatio * 0.5 > imgMainBottom) {
						$magnifier.css({
							top: param.magnifierBdWidth * (-1) + imgMainHeight - currMagnifierHeight + param.zoomScale * imgRatio,
							height: "-=" + param.zoomScale * imgRatio
						});
					...
					if (y - imgMainHeight * zoomRatio * 0.5 < imgMainTop) {
						...
						$imgMagnifier.css({
							top: (y - imgMainTop - (imgMainHeight * zoomRatio) * 0.5) * (-1)
						});
					}
				}
			}
